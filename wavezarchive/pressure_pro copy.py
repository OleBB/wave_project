...
@author: ole
dette dokumentet er laget for å hente ut alle radene i .lvm-filer, som jeg genererte på lab-pcen, 
nå i senere tid gjør jeg all prosseseringen direkte i labview
"""
import pandas as pd
...
#%%
daarlig_navngivning_fs = [read_lvm(f, skip_lines_after_header=2)[['mA']] for f in files]  # select only mA column
# Optionally rename columns so each file has a unique name
for i, f in enumerate(files):
    daarlig_navngivning_fs[i].columns = [f"mA_{i+1}"]
# Concatenate horizontally
df = pd.concat(daarlig_navngivning_fs, axis=1)
