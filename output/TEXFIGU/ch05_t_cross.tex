...
\begin{figure}[htbp]
  ...
  \caption[T\_cross: wind effect via clean nowind reference]{%
    Wind effect on transmission measured three ways: $(OUT/IN)_{nw}$ (blue, nowind baseline); $T_{\mathrm{cross}} = A_{out}^{fw}/A_{in}^{nw}$ (green, fullwind OUT referenced to the clean nowind IN); and $(OUT/IN)_{fw}$ (red, standard fullwind metric). The green--blue gap is the wind effect measured with the clean incident reference. The red--blue gap is the same effect as reported by the standard ratio; the two disagree where the IN probe is contaminated by wind noise at the paddle frequency (CLAUDE.md \S 16). Worst-case divergence between the two metrics: 0.790 vs 0.944 at $f = 1.3$\,Hz, $0.10$\,V. Each subfigure is one input amplitude; $n$ per point in \texttt{analysis\_scratch/t\_cross\_figure\_summary.csv}.
  }
