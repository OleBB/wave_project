...
%   data_class        : DELEG
%   generated_at      : 2026-04-23T11:32:14
%   findings_doc      : 
%
% — Thesis context ──────────────────────────────────────────────
%   chapter           : 04
%   caption_label     : fig:ch04_inspirational_nowind
%   caption_short     : Inspirational time-series overview (tidsserier, uten vind) of a canonical run at $f = 1.4$\,Hz, paddle drive $V = 0.2$\,V, full panel, per240
%
% — Filters ────────────────────────────────────────────────────
%   panel             : full
%   wind              : no
...
%   collapse_panels   : 
%   fft_window_hz     : 0.1
%   extra_params      : run_path=wavedata/20260327-ProbePos4_31_FPV_2-tett6roof-under9Mooring30-height100-lowrange/fullpanel-nowind-amp0200-freq1400-per240-depth580-mstop30-run1.csv, probes=IN:9373/170+OUT:12400/250, zoom=5 periods centred on each probe's H&G window
%
% — Summary stats cited in caption ────────────────────────────
%   stat:ka_in        : 0.125
%   stat:ka_out       : 0.086
%   stat:in_window_s  : [30.69, 37.85]
%   stat:out_window_s : [35.55, 42.71]
%
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\linewidth]{FIGURES/ch04_inspirational_nowind.pdf}
  \caption[Inspirational time-series overview (tidsserier, uten vind) of a canonical run at $f = 1]{
    Inspirational time-series overview (tidsserier, uten vind) of a canonical run at $f = 1.4$\,Hz, paddle drive $V = 0.2$\,V, full panel, per240. Top: incoming wave (probe at 9373/170, IN side of the panel). Bottom: outgoing wave (probe at 12400/250, OUT side). Each probe panel is accompanied by a 5-period zoom centred on that probe's own H\&G analysis window; the OUT zoom sits later in time because the wave group arrives at the OUT probe $\sim\!5$\,s after the IN probe. Measured $ka_{\mathrm{IN}}$ = 0.125, $ka_{\mathrm{OUT}}$ = 0.086. Amber shading on the macro panels marks the zoom interval; the 10-period analysis window is a subset of that interval (see H\&G methodology).
  }
