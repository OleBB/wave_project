...
| `wavescripts/plot_damping_wind_delta_2026-04-28.py` (the cut from `wavescripts/plotter.py`) + `output/FIGURES/ch05_damping_wind_delta_full_A{1,2,3}_2026-04-28.pdf` + `output/TEXFIGU/ch05_damping_wind_delta_2026-04-28.tex` | **Replaced by** `analysis_scratch/wind_effect_table.py` (2026-04-28). The 2-row delta scatter (3 subfigs at 0.1/0.2/0.3 V) showed the same per-(freq, amp) wind-effect numbers as a precision-loss visual; a LaTeX table is more readable for the small cell count (4 freqs × 3 amps = 12 rows) and gives the reader exact %-gain / %-reduction values. Cell wiring at `main_save_figures.py` CH05 §3 swapped from inline `plot_damping_wind_delta(...)` to `_run_delegated_if_missing("analysis_scratch/wind_effect_table.py", …)`. |
| `per40_vs_per240_outin_2026-04-29.{py,csv,pdf,_findings.md}` | **Superseded by** `analysis_scratch/per40_and_per240_HG_shifted.py` (CH04 §4m). The archived script compared per40 (legacy SNARVEI window) against per240 (H&G 10T window) — a *mixed-method* before-picture from the SNARVEI→H&G transition era. The `_HG_shifted` successor applies the probe-shifted Huseby–Grue window to *both* run types and shows they pool to within 1–2 % median agreement. The v2 figure alone proves the pooling is honest; the SNARVEI-vs-H&G "before" picture is methodology archaeology that doesn't need its own thesis figure. Cell wiring at `main_save_figures.py` CH04 §4m swapped from this delegated cell to the HG_shifted one (renumbered §4n → §4m). `FIGURE_CAPTIONS` keys removed from both the full and short caption dicts. |
