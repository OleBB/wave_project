...
%                       — this stub keeps subfigs c+d (decay only); ramp-up
%                       subfigs a+b moved to ch04_wind_rampup_overview.tex.
%
% — Thesis context ──────────────────────────────────────────────
%   chapter           : 04
%   caption_label     : fig:ch04_wind_decay_overview
%   caption_short     :
%
% — Filters ────────────────────────────────────────────────────
%   panel             :
%   wind              : transition (max → 0)
%   amplitude [V]     :
%   frequency [Hz]    :
%   probes            :
%   run_category      :
...
%   collapse_panels   :
%   fft_window_hz     :
%   extra_params      : Single-run time-series, wind decay = 20260327 experimental-fromMaxToZeroWin-depth580-mstop30-run-endofday.csv (probe config march2026_better_rearranging). Probes shown: IN=9373/170 (colour #FEA11B amber, fully exposed to wind), OUT=12400/250 (colour #2EC483 green, sheltered behind panel). Per-probe baseline μ₀ subtracted: mean of last 2 s (tank settled by ~23 min). Y-axis clipped to ±15 mm so the wind-setup tilt and chop envelope are readable on the same scale across both panels. Subfigure order top→bottom: decay full record (minutes), decay first 60 s (seconds).
%
...
\begin{figure}[p]
  ...
  \begin{subfigure}[b]{1.0\linewidth}
    ...
    \caption{Fra full vind til null vind.}
  ...
  \end{subfigure}
  \caption{
    Vindens påvirkning på vannets nivå under nedkjøring fra full vind til null vind. Merk: Ulike x-akser.
  }
  \label{fig:ch04_wind_decay_overview}
\end{figure}
