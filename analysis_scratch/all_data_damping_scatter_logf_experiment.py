...
THESIS_NAME = "ch05_damping_all_data_scatter_logf"
...
# EXPERIMENT (2026-05-08): log-scale bottom axis. Inverts the v2 "cramped at
# low Hz" character — on log-k, low-k ticks are spread, high-k ticks crammed.
# Hz top axis (via add_freq_axis) inherits the log positioning, so Hz labels
# also crowd at the high-f side. Same data, same encoding — only the x-scale
# changes. NOT in main_save_figures; sibling PDF for visual comparison.
ax.set_xscale("log")
...
# Minor ticks: matplotlib's default LogLocator handles log-scale minor ticks
# automatically (subs at 2..9 within each decade). The MultipleLocator(1.0)
# from the linear version doesn't make sense on log-scale.
...
secax.set_xlabel(r"Frekvens (Hz)", fontsize=9)   # Norwegian; siunitx-style
                                                 ...
                                                 # text.usetex=True.
from matplotlib.ticker import FixedLocator, FixedFormatter, NullLocator, ScalarFormatter
_used_freqs = sorted(wave_clip["WaveFrequencyInput [Hz]"].unique())
# Override the inherited log auto-formatter (which was rendering high-Hz
# ticks as "1.9 × 10⁰" gibberish) with explicit positions and labels.
secax.xaxis.set_major_locator(FixedLocator(_used_freqs))
secax.xaxis.set_major_formatter(FixedFormatter([f"{f:.1f}" for f in _used_freqs]))
secax.xaxis.set_minor_locator(NullLocator())
secax.tick_params(labelsize=7)

# Bottom k-axis: log scale would default to decade-only labels (10⁰, 10¹).
# Force plain integer labels at every k integer in the data range so the
# ruler stays readable.
_k_max = int(np.ceil(wave_clip["k"].max()))
_k_ticks = list(range(2, _k_max + 1))
...
# ka range per wind condition appended to the Vind labels — same data subset
# the scatter actually plots. Prefer IN ka (FFT) from meta if present,
# otherwise compute from k × IN-amplitude with mm→m conversion.
_ka_ranges = {}
for _w in ["no", "full"]:
    ...
    if len(_ka_vals):
        _ka_ranges[_w] = (float(_ka_vals.min()), float(_ka_vals.max()))
    else:
        _ka_ranges[_w] = None
...
def _wind_label_with_ka(w):
    ...
    if rng is None:
        return WIND_LABEL[w]
    return f"{WIND_LABEL[w]}  ($ka$: {rng[0]:.3f}–{rng[1]:.3f})"
