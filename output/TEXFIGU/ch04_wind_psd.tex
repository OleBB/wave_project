...
\begin{figure}[htbp]
  ...
  \caption[POWER spectral density (PSD) of the free surface at each wave gauge during wind-only runs (no paddle waves)]{
    POWER spectral density (PSD) of the free surface at each wave gauge during wind-only runs (no paddle waves). All 30 nowave runs overlaid; colour encodes wind condition. Stillwater runs (no wind) shown as baseline. Wind energy is concentrated above 2\,Hz — the paddle frequency range (0.65--1.9\,Hz) is unaffected.
  }
