...
CHAPTER 04 — METHODOLOGY
  ...
  §4p   ch04_wind_rampup_overview        [DELEG] ~  Wind ramp-up (full + 60 s zoom — IN/OUT only, μ₀-subtracted, ±15 mm)
        ...
        ch04_wind_decay_overview         [DELEG] ~  Wind decay (full + 60 s zoom — IN/OUT only, μ₀-subtracted, ±15 mm)
...
FIGURE_CAPTIONS: dict[str, str] = {
    ...
    "ch04_wind_decay_overview":        "Vindens påvirkning på vannets nivå under nedkjøring fra full vind til null vind. Merk: Ulike x-akser.",
...
# Both parents (rampup + decay) are produced by the same script — call it once
# with the union of expected outputs; the deleg machinery skips re-running if
# all targets already exist on disk.
_run_delegated_if_missing(
    ...
    [Path("output/FIGURES/ch04_wind_rampup_full.pdf"),
     ...
     Path("output/TEXFIGU/ch04_wind_rampup_overview.tex"),
     Path("output/TEXFIGU/ch04_wind_decay_overview.tex")],
    label="ch04_wind_rampup_overview + ch04_wind_decay_overview",
)
