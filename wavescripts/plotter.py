...
def _draw_damping_freq_ax(
    ...
    for amp in sorted(subset[GC.WAVE_AMPLITUDE_INPUT].unique()):
        ...
        ax.errorbar(
            freq_to_k(amp_data[GC.WAVE_FREQUENCY_INPUT].values),
...
# WIND DELTA section (3 functions) archived 2026-04-28 →
# ignore_this_archive/wavescripts/plot_damping_wind_delta_2026-04-28.py
# Replaced by analysis_scratch/wind_effect_table.py (LaTeX table generator).
        ...
        for wind in wind_order:
            ...
            for i, amp in enumerate(amps):
                ...
                if scatter:
                    ...
                    ax.scatter(
                        k_vals, amp_grp["parallel_ratio"].values,
                    ...
                    ax.errorbar(
                        freq_to_k(stats.index.values), stats["mean"],
    ...
    def _make_fig() -> plt.Figure:
        ...
        for ax, pos in zip(axes, probes_in):
            ...
            for wc, grp in sub.groupby("wind"):
                ...
                ax.plot(freq_to_k(agg.index.values), agg.values, marker="o", lw=1.2, markersize=6,
    ...
    def _make_fig() -> plt.Figure:
        ...
        for ci, pos in enumerate(probes_avail):
            ...
            for wc, grp in sub.groupby("WindCondition"):
                ...
                if scatter:
                    k_vals = freq_to_k(grp["WaveFrequencyInput [Hz]"].values)
                    ax_rt.scatter(k_vals, grp["ratio"].values,
                                  s=12, alpha=0.45, color=c, label=wc)
                else:
                    agg = grp.groupby("WaveFrequencyInput [Hz]")["ratio"].median()
                    ax_rt.plot(freq_to_k(agg.index.values), agg.values,
...
# ═══════════════════════════════════════════════════════════════════════════════
# FFT PEAK-BIN BIAS & OUT/IN CANCELLATION
# ═══════════════════════════════════════════════════════════════════════════════

def plot_fft_peak_bias_cancellation(
    plotvariables: dict,
    chapter: str = "04",
) -> Optional[plt.Figure]:
    """
    FFT peak-bin bias: nearest-bin AFFT is biased by paddle-drift vs bin-grid,
    but the OUT/IN ratio cancels the bias to <1% on average.

    Reads precomputed per-run amplitudes (nearest-bin and parabolic-interpolated
    sub-bin peak) from:

        analysis_scratch/fft_peak_bias_outin_impact.csv

    generated by `analysis_scratch/fft_peak_bias_outin_impact.py`. Regenerate
    that CSV after any pipeline change that alters analysis-window placement
    or amplitude extraction.

    Produces a 2x2 thesis-polished figure:

        (a) OUT/IN(nearest) vs OUT/IN(parabolic) scatter, colored by wind
        (b) Distribution of Δ(OUT/IN) = parabolic - nearest
        (c) Δ vs paddle frequency
        (d) Δ vs paddle peak-offset at the IN probe (mHz from nominal)

    Parameters
    ----------
    plotvariables : dict
        Standard plot-config dict with 'filters' and 'plotting' keys.
        The "plotting" key may set "csv_path" to override the default
        cache location.
    chapter : str
        Chapter prefix, defaults to "04".
    """
    plotting   = plotvariables.get("plotting", {})
    show_plot  = plotting.get("show_plot",  False)
    save_plot  = plotting.get("save_plot",  False)
    ...
    default_csv = Path(__file__).resolve().parent.parent / \
                  "analysis_scratch" / "fft_peak_bias_outin_impact.csv"
    csv_path = Path(plotting.get("csv_path", default_csv))

    if not csv_path.exists():
        print(f"  plot_fft_peak_bias_cancellation: cache not found — {csv_path}")
        print(f"  Regenerate with: "
              f"python analysis_scratch/fft_peak_bias_outin_impact.py")
        return None

    df = pd.read_csv(csv_path)
    n_total = len(df)
    abs_delta = df["delta_outin"].abs()
    mean_abs_rel = (abs_delta / df["outin_nearest"].abs()).mean()
    frac_lt_002  = (abs_delta < 0.02).mean()
    max_abs = abs_delta.max()
    n_nowind   = int((df["wind"] == "no").sum())
    n_fullwind = int((df["wind"] == "full").sum())
    fw_mean_delta = df.loc[df["wind"] == "full", "delta_outin"].mean()
    nw_mean_delta = df.loc[df["wind"] == "no",   "delta_outin"].mean()

    _slots = {
        "n_total":      n_total,
        "n_nowind":     n_nowind,
        "n_fullwind":   n_fullwind,
        "mean_abs_rel": f"{100*mean_abs_rel:.2f}\\,\\%",
        "frac_lt_002":  f"{100*frac_lt_002:.0f}\\,\\%",
        "max_abs":      f"{max_abs:.3f}",
    ...
    def _make_fig() -> plt.Figure:
        ...
        wind_order = [("no", "nowind"), ("full", "fullwind")]

        # (a) scatter OUT/IN nearest vs parabolic
        ax = axes[0, 0]
        for wind, label in wind_order:
            sub = df[df["wind"] == wind]
            if sub.empty:
                continue
            color = WIND_COLOR_MAP.get(wind, "grey")
            ax.scatter(sub["outin_nearest"], sub["outin_parabolic"],
                       s=10, alpha=0.55, color=color,
                       label=f"{label} (n={len(sub)})", edgecolor="none")
        lim = max(df["outin_nearest"].max(), df["outin_parabolic"].max()) * 1.05
        ...
        ax = axes[0, 1]
        bin_edges = np.linspace(df["delta_outin"].min(),
                                df["delta_outin"].max(), 50)
        for wind, label in wind_order:
        ...
        # (d) delta vs paddle peak-offset at IN probe (mHz)
        ax = axes[1, 1]
        for wind, label in wind_order:
            sub = df[df["wind"] == wind].copy()
            if sub.empty:
                continue
            color = WIND_COLOR_MAP.get(wind, "grey")
            offset_mHz = (sub["f_in_peak"] - sub["freq_hz"]) * 1000.0
            ax.scatter(offset_mHz, sub["delta_outin"],
                       s=10, alpha=0.55, color=color, label=label,
                       edgecolor="none")
    ...
    if save_plot:
        meta = build_fig_meta(
            ...
            extra={
                ...
                "data":    str(csv_path.relative_to(csv_path.parent.parent)),
                "n_runs":  n_total,
            },
        )
