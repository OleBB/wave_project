...
_caption = (
    "Top row: OUT/IN(FFT) vs FFT analysis-window length (paddle periods) "
    "at 0.10, 0.20 and 0.30\\,V input. A flat curve means the transmission "
    "metric is insensitive to window choice within the stable plateau "
    f"(worst drift across all (amplitude, wind) cells: {worst_drift:.3f}). "
    "Bottom row: IN probe amplitude under no-wind (blue), full-wind raw "
    "(red) and full-wind after wind-subtraction at the paddle bin (green). "
    "The wind correction uses "
    "$E[A_{\\text{wind}}^2] = 2\\!\\int_{f_p\\pm 0{.}05}$\\,PSD_{\\text{wind}}\\,df$ "
    "with PSD_{\\text{wind}} = mean PSD_{\\text{residual}}(full) $-$ "
    "mean PSD_{\\text{residual}}(no) per (freq, amp, mooring) group. "
    "Agreement between green (corrected full-wind) and blue (no-wind) "
    f"within ${corr_match_median*100:.1f}\\%$ median "
    f"(worst ${corr_match_max*100:.1f}\\%$) validates the T_cross "
    "assumption that the paddle output is wind-independent. "
    f"Median wind contamination fraction $A_{{\\text{{wind}}}} / A_{{\\text{{in,fw}}}}$ "
    f"= ${contam_med*100:.1f}\\%$ (max ${contam_max*100:.1f}\\%$)."
)
...
# ── 9. Findings markdown ─────────────────────────────────────────────────────
findings = f"""# CH04 §2.6 — Paddle contamination & window-size sensitivity — findings

Generated by `analysis_scratch/paddle_contamination_study.py`.
Scope: full panel, {min(FREQS):.1f}–{max(FREQS):.1f} Hz, {AMPS} V,
mooring `below_90_loose` (230+300 pooled), quality_flag in {{ok, probe_malfunction_secondary}}.

## Part A — Window-size sensitivity

How does OUT/IN (FFT) move as we shrink the FFT analysis window from the
full stable plateau down to 20 paddle periods?

Worst drift (max − min across N_tags) in median OUT/IN across all
(amp, wind) cells: **{worst_drift:.4f}**

If the whole table is flat within run-to-run scatter, the thesis metric
is robust. If fullwind drifts more than nowind, that's the signature of
wind non-stationarity bleeding into the FFT via coherence loss.

## Part B — Wind-correction at the paddle bin

For each (freq, amp, mooring) group with matched no-wind/full-wind
coverage, we estimated the wind PSD at the paddle bin via

    PSD_wind(f)  =  mean PSD_residual(full)  −  mean PSD_residual(no)

integrated over ±0.05 Hz around the paddle → E[A_wind²]. Incoherent
subtraction gives A_in_corrected = √max(0, A_in_fw² − E[A_wind²]).

Median contamination fraction √(A_wind²) / A_in_fw: **{contam_med*100:.2f} %**
(max **{contam_max*100:.2f} %**).

## Part C — T_cross cross-check

If the correction is sound and the paddle is wind-independent, then
A_in_corrected(full) ≈ A_in(no). Agreement statistics (across all
matched runs, {len(_match_valid)} rows):
...
- If drift is small, contamination is small, and corrected matches
  no-wind: the CH05 OUT/IN numbers are a clean transmission metric and
  T_cross is a consistency check, not a correction.
- If drift is non-trivial or contamination > 10 % at any (freq, amp): we
  need to either (i) report the correction as a systematic, or (ii)
  adopt `A_corrected` as the primary metric and move raw `A_fw` to a
