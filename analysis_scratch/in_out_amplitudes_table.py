# %%
"""
IN/OUT FFT amplitude table — ch05_in_out_amplitudes_vs_freq_table.
==================================================================

Sibling of ch04_plateau_values: same per-run probe-shifted window FFT
amplitudes, same cohort (canon March-2026 cond4 lowrange), but the
output table reports A_IN ± σ(A_IN) and A_OUT ± σ(A_OUT) for each
(f, amp, wind) cell — no K_t, no σ(K_t). The focus is the absolute
amplitudes on each side of the panel; the ratio lives elsewhere
(plateau_values, damping_freq).

Windowing (matches plateau_values_table):

    t_start(probe, f) = r_probe / c_g(f, h) + N_OFFSET / f
    t_end(probe, f)   = t_start + N_LENGTH / f

with N_OFFSET = 7, N_LENGTH = 10. Probe-shifted: A_IN evaluates the IN
signal over its own window; A_OUT evaluates the OUT signal over its own
(later) window.

Per (f, amp, wind) cell:
  * n       — number of canon runs
  * A_IN    — mean across runs of per-run window-FFT amplitude (mm)
  * A_IN_sd — run-to-run std of A_IN (around the mean, ddof=1)
  * A_OUT   — same for OUT
  * A_OUT_sd — same for OUT

24 cells = 4 freqs × 3 amps × 2 winds. Grouped by amp tier in the
LaTeX output.

Outputs:
    output/TABLES/data/ch05_in_out_amplitudes_vs_freq_table.csv
    output/TABLES/data/ch05_in_out_amplitudes_vs_freq_table.meta.json
    output/TABLES/ch05_in_out_amplitudes_vs_freq_table.tex
    analysis_scratch/in_out_amplitudes_per_run.csv (audit-trail companion)

Caption text is read from TABLE_CAPTIONS in main_save_tables.py.
"""
...
THESIS_NAME  = "ch05_in_out_amplitudes_vs_freq_table"
CHAPTER      = "05"
SCRIPT_REL   = "analysis_scratch/in_out_amplitudes_table.py"

DATA_DIR     = BASE / "output" / "TABLES" / "data"
RENDER_CSV   = DATA_DIR / f"{THESIS_NAME}.csv"
META_JSON    = DATA_DIR / f"{THESIS_NAME}.meta.json"
SCRATCH_CSV  = Path(__file__).parent / "in_out_amplitudes_per_run.csv"
...
agg = (
    ...
    .agg(
        n         = ("A_in_mm",  "size"),
        A_in_mm   = ("A_in_mm",  "mean"),
        A_in_sd   = ("A_in_mm",  "std"),
        A_out_mm  = ("A_out_mm", "mean"),
        A_out_sd  = ("A_out_mm", "std"),
    )
    .reset_index()
)
...
render_df = agg.rename(columns={
    ...
    "A_in_mm":   "A_in",
    "A_out_mm":  "A_out",
})[["amp_v", "freq", "wind", "n", "A_in", "A_in_sd", "A_out", "A_out_sd"]]
