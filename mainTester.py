...
print("plotconditions = ", plotconditions)
...
def filter_chosen_files_old(plotvariables):
 # --- Filtering based on requested conditions ---
    df_sel = meta.copy()
    if plotvariables["amp"] is not None:
        df_sel = df_sel[df_sel["WaveAmplitudeInput [Volt]"] == plotvariables["amp"]]
    if freq is not None:
        df_sel = df_sel[df_sel["WaveFrequencyInput [Hz]"] == freq]
    if wind is not None:
        df_sel = df_sel[df_sel["WindCondition"] == wind]
    if tunnel is not None:
        df_sel = df_sel[df_sel["TunnelCondition"] == tunnel]
    if mooring is not None:
        df_sel = df_sel[df_sel["Mooring"] == mooring]
    if df_sel.empty:
        print("No matching datasets found.")
