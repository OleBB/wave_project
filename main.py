...
for i, data_path in enumerate(dataset_paths):
    ...
    try:
        ...
        dfs, meta = load_or_update(
            data_path, force_recompute=force, total_reset=total_reset
        )
        ...
        print(
            f"Successfully processed {len(meta_sel)} selections from {data_path.name}"
        )
...
damping_filtrert = filter_for_damping(
    damping_groupedruns_df, dampingplotvariables["filters"]
)
...
filtrert_frequencies = filter_for_frequencyspectrum(
    combined_meta_sel, freqplotvariables
)
